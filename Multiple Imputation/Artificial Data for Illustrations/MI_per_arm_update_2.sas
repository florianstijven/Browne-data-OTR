...
data final_updated2_moderate;
set indata.final_updated2_moderate;
run;

data final_updated2_strong;
set indata.final_updated2_strong;
run;
...
data final_updated_trans2_moderate;
set final_updated2_moderate;
...
data final_updated_trans2_strong;
set final_updated2_strong;
madrsv1 = madrsv1*100/60 + 1; madrst2 = madrst2*100/60 + 1; madrst3 = madrst3*100/60 + 1; madrst4 = madrst4*100/60 + 1;
cesd = cesd*100/60 + 1; cesd2 = cesd2*100/60 + 1; cesd3 = cesd3*100/60 + 1; cesd4 = cesd4*100/60 + 1;
vas = vas + 1; vas2 = vas2 + 1; vas3 = vas3 + 1; vas4 = vas4 + 1;
sasb = (sasb - 1)*100/4 + 1; sasb2 = (sasb2 - 1)*100/4 + 1; sasb3 = (sasb3 - 1)*100/4 + 1; sasb4 = (sasb4 - 1)*100/4 + 1; 
...
/* Proc MI: For group = 1 */
ods graphics on;
proc mi data=final_updated_trans2_moderate out=final_MI_1_updated2_moderate nimpute=&n_imputations seed=1;		
	where group = 1;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
ods graphics on;
proc mi data=final_updated_trans2_strong out=final_MI_1_updated2_strong nimpute=&n_imputations seed=1;		
	where group = 1;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
/* Proc MI: For group = 2 */
ods graphics on;
proc mi data=final_updated_trans2_moderate out=final_MI_2_updated2_moderate nimpute=&n_imputations seed=1;		
	where group = 2;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
ods graphics on;
proc mi data=final_updated_trans2_strong out=final_MI_2_updated2_strong nimpute=&n_imputations seed=1;		
	where group = 2;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
/* Proc MI: For group = 3 */
ods graphics on;
proc mi data=final_updated_trans2_moderate out=final_MI_3_updated2_moderate nimpute=&n_imputations seed=1;		
	where group = 3;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
ods graphics on;
proc mi data=final_updated_trans2_strong out=final_MI_3_updated2_strong nimpute=&n_imputations seed=1;		
	where group = 3;
	class sex disorder phealth;
	fcs nbiter=&n_burnin plots=trace reg() logistic() logistic(disorder/link=glogit);
	var sex disorder phealth age age_sq numchild
...
data final_MI_updated2_moderate;
	set final_MI_1_updated2_moderate final_MI_2_updated2_moderate final_MI_3_updated2_moderate;
run;

data final_MI_updated2_strong;
	set final_MI_1_updated2_strong final_MI_2_updated2_strong final_MI_3_updated2_strong;
run;

/* We transform the variables in the full imputed data set back to the original scale. */
data final_MI_updated2_moderate;
set final_MI_updated2_moderate (drop = age_sq);
...
data final_MI_updated2_strong;
set final_MI_updated2_strong (drop = age_sq);
madrsv1 = (madrsv1 - 1)*60/100; madrst2 = (madrst2 - 1)*60/100; madrst3 = (madrst3 - 1)*60/100; madrst4 = (madrst4 - 1)*60/100;
cesd = (cesd - 1)*60/100; cesd2 = (cesd2 - 1)*60/100; cesd3 = (cesd3 - 1)*60/100; cesd4 = (cesd4 - 1)*60/100;
vas = vas - 1; vas2 = vas2 - 1; vas3 = vas3 - 1; vas4 = vas4 - 1;
sasb = (sasb - 1)*4/100 + 1; sasb2 = (sasb2 - 1)*4/100 + 1; sasb3 = (sasb3 - 1)*4/100 + 1; sasb4 = (sasb4 - 1)*4/100 + 1; 
...
proc sort data=final_MI_updated2_moderate;
	by _imputation_;
run;

proc sort data=final_MI_updated2_strong;
	by _imputation_;
run;
...
data final_MI_updated2_moderate;
set final_MI_updated2_moderate;
...
data out.final_MI_updated2_moderate;
set final_MI_updated2_moderate;
FORMAT _all_;
INFORMAT _all_;
run;

data out.final_MI_updated2_strong;
set final_MI_updated2_strong;
FORMAT _all_;
INFORMAT _all_;
run;
