...
/* Update 2: add quadratic effect of age. */ 
data final_update2;
set final_update1;
if cesd2 ^= . then
	do; 
...
/* Update 1: add three-way interaction. */ 
data final_update3;
set final_update1;
if cesd2 ^= . then
	do; 
...
/* Store the three updated data sets. The formatting is removed because this can cause issues when the data are opened in R or other software. */
...
data out.final_updated2;
set final_update2;
FORMAT _all_;
INFORMAT _all_;
run;

data out.final_updated3;
set final_update3;
FORMAT _all_;
INFORMAT _all_;
run;
