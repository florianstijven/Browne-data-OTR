...
(interaction) effects are not present there. We consider three artificial data
sets.
...
2. In addition to the changes in the first point, we add a quadratic effect for
the main effect of age. So, the outcome regression model that we have used
before will be misspecified.
3. In addition to the changes in the first point, we add a three-way interaction
for treatment-past MDD-age. The outcome regression model does not include
three-way interactions and will thus again be missspecified. In addition, this
three-way interaction is not captured by the class of linear treatment regimes
...
outcome variable is as follows,
...
```{r}
lm_fit2_global = lm(global_formula, data_update2)
lm_fit2_per_arm = lm(per_arm_formula, data_update2)
modelsummary::msummary(
  list("No Interactions" = lm_fit2_global,
       "Treatment-Interactions" = lm_fit2_per_arm),
  estimate = "{estimate} ({std.error}){stars}",
  statistic = NULL,
  notes = "+, p < 0.1; *, p < 0.05; **, p < 0.01; ***, p < 0.001.",
  gof_map = c("r.squared", "adj.r.squared")
...
The third update involves the introduction of additional treatment effect
heterogeneity that leads the our outcome regression model being misspecified.
The modification of the outcome variable is as follows,
$$\tilde{\texttt{cesd2}} = \texttt{cesd2} - 0.2 \cdot (\texttt{age} - 50) \cdot \texttt{past_MDD} \cdot I(\texttt{group} = 1) $$
where a tilde indicates the updated value and $\texttt{cesd2}$ is the value from
the **first** update. This corresponds to a three-way interaction. Because of
this, the outcome regression model will be misspecified. In contrast to update
2, the misspecification is now in the interaction terms instead of the main
effects (as was the case for update 2). This also means that the class of linear
regimes that we are considering does not contain the optimal regime.

```{r}
data_update3 = data_update1 %>%
  mutate(
    cesd2 = ifelse(
      group == "Sertraline and IPT",
      cesd2 - 0.2 * (age - 50) * (past_MDD == "Yes"),
      cesd2
    ),
    change_cesd = cesd - cesd2
  )
...
We next repeat the same regression models and plots as before. The other plots
ar similar to those from update 1, although there now is an additional three-way
interaction.

```{r}
lm_fit3_global = lm(global_formula, data_update3)
lm_fit3_per_arm = lm(per_arm_formula, data_update3)
modelsummary::msummary(
  list("No Interactions" = lm_fit3_global,
       "Treatment-Interactions" = lm_fit3_per_arm),
  estimate = "{estimate} ({std.error}){stars}",
  statistic = NULL,
  notes = "+, p < 0.1; *, p < 0.05; **, p < 0.01; ***, p < 0.001.",
  gof_map = c("r.squared", "adj.r.squared")
