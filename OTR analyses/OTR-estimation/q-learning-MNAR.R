...
imputed_data_file_updated1 = "Multiple Imputation/MNAR-sensitivity-analysis/final_mi_global_updated1.sas7bdat"
...
imputed_data_updated1 = read.sas7bdat(imputed_data_file_updated1)
...
# Transform shift variable onto the correct scale and as a factor variable.
imputed_data %>%
  mutate(
    shift = factor(shift / 10)
  )
imputed_data %>% head()
...
# Filter on the outcome cesd. For the sensitivity analysis, we will only use
# cesd as outcome variable. The data are further split according to imputation
# number, shift, and imputation method. The first two columns of the new tibble
# contain the imputation number and outcome variable, respectively.
imputed_data_reformatted = imputed_data_long_6months %>%
  filter(outcome == "cesd") %>%
  group_by(X_Imputation_, data, shift) %>%
...
saveRDS(object = results_tbl, file = paste0(saveto, "OTR_q_results_global_shift.rds"))
