...
# For each outcome, we only use the optimal tuning parameters that were found
# previously. For CESD, MADRS, and FAMFUN this is a population size of 500 with
# 20 independent runs of the GA. For VAS and SAS, this is a population size of
# 3000 with 5 independent runs. 
imputed_data_reformatted =
  bind_rows(
    # imputed_data_reformatted %>%
    #   filter(outcome %in% c("cesd", "madrs", "famfun")) %>%
    #   cross_join(y = tibble(seed = c(101:120))) %>%
    #   mutate(population_size = 500),
    imputed_data_reformatted %>%
      filter(outcome %in% c("sas", "vas")) %>%
...
OTR_estimated_list = plyr::llply(
  .data = imputed_data_reformatted_list[1:100],
...
results_tbl = imputed_data_reformatted[1:100, ] %>%
  select(-data_set)
results_tbl$OTR_estimated = OTR_estimated_list

saveRDS(object = results_tbl, file = paste0(saveto, "OTR_results.rds"))