...
In this document, we examine the influence of different data analytic choices
...
```
For completeness, we also summarize the information in the above figure in the 
following table.
...
pooled_inference_aggregated_rules_tbl %>%
  ...
  mutate(
    lower_ci = pooled_estimated_value - 1.96 * sqrt(pooled_total_variance),
    upper_ci = pooled_estimated_value + 1.96 * sqrt(pooled_total_variance)
  ) %>%
  select(imputation,
         OTR_method,
