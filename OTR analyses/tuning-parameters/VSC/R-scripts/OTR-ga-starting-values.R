...
# Add a column with the type of starting values as variable.
imputed_data_reformatted = imputed_data_reformatted %>%
  cross_join(y = tibble(starting_value = c("Q", "zero", "random")))
...
saveRDS(object = results_tbl, file = paste0(saveto, "OTR_results-starting-values.rds"))
