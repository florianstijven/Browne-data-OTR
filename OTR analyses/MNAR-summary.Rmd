...
pooled_inference_aggregated_rules_tbl %>%
  ...
  mutate(OTR_method = forcats::fct_expand(
    OTR_method,
    c("Value Search Estimation")),
    OTR_method = fct_relevel(
      OTR_method,
      c(
        "Q-learning",
        "Sertraline",
  ...
  ggplot(aes(x = shift_cesd, y = pooled_estimated_value, color = OTR_method)) +
  geom_point(aes(color = OTR_method), position = position_dodge(1.5)) +
  geom_errorbar(
    aes(
      ymin = pooled_estimated_value - 1.96 * sqrt(pooled_total_variance),
      ymax = pooled_estimated_value + 1.96 * sqrt(pooled_total_variance)
  ...
  ylab(latex2exp::TeX("Pooled Estimated Value, $\\bar{\\hat{\\nu}}(\\tilde{d})")) +
  xlab(latex2exp::TeX("$\\bar{\\Delta}_{MNAR}$")) +
  scale_color_viridis_d(
    name = "OTR method",
    drop = FALSE,
    breaks = c("Sertraline", "Sertraline + IPT", "Q-learning")
  ) +
  theme(
...
ggsave(
  filename = "figures-manuscript/main-text/mnar-estimated-values.pdf",
  device = "pdf",
  width = single_width,
...
classification_aggregated_rules_tbl %>%
  ...
  xlab(latex2exp::TeX("$\\bar{\\Delta}_{MNAR}$")) +
  ylab("Cohen's kappa")
ggsave(filename = "figures-manuscript/main-text/mnar-cohen-kappa.pdf",
       device = "pdf",
       width = single_width,
       height = single_height,
       units = "cm",
...
allocation_tbl %>%
  filter(data == "Original") %>%
  mutate(shift_cesd_cat = paste0("bar(Delta)[MNAR] ==", round(shift_cesd, 1))) %>%
  arrange(shift_cesd) %>%
  mutate(shift_cesd_cat = forcats::fct_inorder(shift_cesd_cat)) %>%
  ggplot(aes(
    x = abs(C_hat_or),
    group = ifelse(different_classification == "Same Classification", "A", "B"),
    fill = different_classification
  ...
  scale_fill_viridis_d(name = "Differential classification") +
  facet_wrap(~ shift_cesd_cat, labeller = label_parsed) +
  theme(legend.position = "bottom",
        legend.title = element_blank())
ggsave(
  filename = "figures-manuscript/main-text/mnar-distribution-contrasts.pdf",
  device = "pdf",
  width = double_width,
