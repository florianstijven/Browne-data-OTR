...
Therefore, for each
shift on the $\tilde{\texttt{cesd}}$ scale, we also compute the average difference between the imputed values under MAR and the MNAR scenario. Specifically, this is done by
computing
...
regime is relatively insensitive to a global shift.
  * For the original Browne data, the estimated difference in values of the         aggregated
  and best one-size-fits-all regime depends on the global shift only for
  ...
  * For the updated Browne data, the estimated difference in values of the        aggregated
  and best one-size-fits-all regime is practically independent of the global
...
* In the original Browne data, Cohen's kappa is very sensitive to the global shift.
* In the updated Browne data, Cohen's kappa is relatively insensitive to 
the global shift. 
...
regimes remain close to the estimated value of the best one-size-fits-all
regime. 

In the updated Browne data, the aggregated regime is relatively insensitive 
to the MNAR scenarios with a global shift. Both the classification of patients 
and the aggregated regime's value (relative to the trivial regimes) are not 
severely changed under these MNAR scenarios.
