...
allocation_tbl = allocation_tbl %>%
  mutate(different_classification = allocation != allocation_mar)
# Add shifts on the cesd scale that were computed before. 
allocation_tbl = allocation_tbl %>%
  left_join(shift_to_cesd_scale_tbl,
            by = c("shift", "data"))
...
```{r contrasts-diff-classification-aipwe, fig.cap="Distribution of the AIPWE-based estimates of the patient-level contrasts. Each subplot shows the histogram for these estimated contrasts, colored by whether the patient was differentially classified as compared to the MAR scenario. The subplot titles indicate the average shift in the corrsponding MNAR scenario."}
allocation_tbl %>%
  filter(data == "Original") %>%
  ggplot(aes(x = abs(C_hat), fill = different_classification)) +
...
allocation_tbl %>%
  filter(data == "Original") %>%
  ggplot(aes(x = abs(C_hat_or), fill = different_classification)) +
  geom_histogram(color = "black", position = "identity", alpha = 0.5) +
  ylab("Count (number of points in bin)") +
  xlab("Absolute value of estimated patient-level contrast") +
  scale_fill_discrete(name = "Differential classification") +
