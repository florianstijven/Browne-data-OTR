...
OTR_results %>%
  ...
  mutate(norm = sqrt(constant ^ 2 + sex ^ 2 + age ^ 2 + cesd ^ 2 + famfun ^ 2 + past_MDD ^
                       2)) %>%
  # Divide the regime parameters by the norm.
  mutate(
    constant = constant / norm,
...
OTR_results %>%
  ...
  mutate(
    ...
    OTR_method = as.factor(OTR_method),
    OTR_method = forcats::fct_expand(OTR_method, c("Sertraline", "Sertraline + IPT"))
  ) %>%
  # Divide the regime parameters by the norm.
  mutate(
    constant = constant / norm,
  ...
  scale_color_viridis_d(drop = FALSE,
                        breaks = c("Q-learning", "Value Search Estimation")) +
  scale_fill_viridis_d(drop = FALSE,
                       breaks = c("Q-learning", "Value Search Estimation")) +
  theme(legend.position = "bottom", legend.title = element_blank())
ggsave(
  filename = "figures-manuscript/main-text/between-imputation-pm-estimates.pdf",
  device = "pdf",
  width = double_width,
